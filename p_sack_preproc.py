# p_sack_preproc.py (Deferred Imports for Debugging)

import pandas as pd
import numpy as np
import re # Keep this top-level import
from typing import List, Optional, Dict, Any # Keep this top-level import
...
#  as in p_sack_preproc_py_imported_re_02)
    ...
    for item in relevant_data:
        ...
        if len(row_buffer) == num_headers:
            ...
            if header_pattern.match(first_cell_text):
                 if len(row_buffer) > 1 and all(header_pattern.match(str(c).strip()) or '%' in str(c) for c in row_buffer[1:]):
