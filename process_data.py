...
# and saves the final combined DataFrame to a new CSV.
...
import os
import glob
import pytz # Keep for potential future use, though not strictly needed here
import traceback
import re
...
BETCENTER_CSV_PATTERN = "betcenter_odds_*.csv" # Use the non-local filename now
...
def load_and_prepare_sackmann_data(csv_filepath: str) -> Optional[pd.DataFrame]:
    ...
    try:
        ...
        # Keep original name for display column
        ...
        # Ensure SACKMANN_COLS_KEEP includes the display TournamentName
        cols_to_keep_final = list(set(SACKMANN_COLS_KEEP + ['TournamentName'])) # Use set to avoid duplicates
        df_out = df[[col for col in cols_to_keep_final if col in df.columns]].copy()
...
def load_and_prepare_betcenter_data(csv_filepath: str) -> Optional[pd.DataFrame]:
    ...
    try:
        ...
        cols_to_select = ['TournamentName'] + MERGE_KEY_COLS + ['p1_odds', 'p2_odds'] # Select original odds names
...
def merge_data(sackmann_df: pd.DataFrame, betcenter_df: Optional[pd.DataFrame]) -> pd.DataFrame:
    ...
    # Keep all Sackmann data, add Betcenter where possible
    if betcenter_df is None or betcenter_df.empty:
        ...
        try:
            ...
            if not unmatched_sackmann.empty:
                ...
                # Merge only the unmatched subset with swapped betcenter data
                ...
                if not swapped_matches_found.empty:
                     # Update the original merged_df using the index (keys)
    ...
    # --- Calculate Spread on the final DataFrame ---
    ...
    final_df['p1_spread'] = final_df['bc_p1_odds'] - final_df['Player1_Match_Odds']
    final_df['p2_spread'] = final_df['bc_p2_odds'] - final_df['Player2_Match_Odds']
    ...
    print(final_df[['TournamentName', 'Player1Name', 'Player2Name', 'Player1_Match_Odds', 'bc_p1_odds', 'p1_spread']].head())
    return final_df

# --- Main Execution Logic ---
if __name__ == "__main__":
    print("="*50)
    print(" Starting Data Processing (Sackmann + Betcenter)...")
    print("="*50)
    ...
    if latest_sackmann_csv:
        ...
        print("CRITICAL: Sackmann data file not found. Cannot proceed.")
    ...
    if latest_betcenter_csv:
        ...
        print("Warning: Betcenter data file not found. Proceeding with Sackmann data only.")
    ...
    if final_merged_data is not None and not final_merged_data.empty:
        ...
        try:
            final_merged_data.to_csv(output_path, index=False, encoding='utf-8')
            print(f"Successfully saved merged data to: {output_path}")
        except Exception as e:
            print(f"Error saving merged data to CSV: {e}")
            traceback.print_exc()
    else:
        print("\nNo final data generated to save.")
