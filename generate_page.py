# generate_page.py (v16 - Fix Styler KeyError)
# Adds reset_index() before applying styles to prevent non-unique index errors.
    ...
    try:
        ...
        try: # Sorting
            ...
            if sort_cols:
                ...
                df_numeric = df_numeric.loc[df_display.index] # Reorder numeric based on display sort
        ...
        # --- FIX: Reset index before styling ---
        ...
        return html_table
         ...
         # Catch the specific error if it persists even after reset_index
...
def generate_strategy_log_table(df_log: pd.DataFrame) -> str:
    ...
    try:
        ...
        for col in cols_to_display:
             ...
             except (ValueError, IndexError): header_map[col] = col
        ...
        # Reset index here too for consistency, although less likely to cause issues
        styler.data = styler.data.reset_index(drop=True)
        html_table = styler.to_html(index=False, escape=True, na_rep='-', border=0)
