...
# *** UPDATED CSV PATTERN ***
CSV_PATTERN = "sackmann_matchups_*.csv" # Look for the new matchup CSVs
TEMPLATE_FILE = "index.html"
OUTPUT_HTML_FILE = "index.html"

# Expected columns in the matchup CSV generated by p_sack_preproc.py
EXPECTED_COLS = [
    'TournamentName', 'Round', 'Player1Name', 'Player2Name',
    'Player1_Match_Prob', 'Player2_Match_Prob',
    'Player1_Match_Odds', 'Player2_Match_Odds'
    # Add 'TournamentURL' if needed for links, 'ModelName' if displaying
]
...
def generate_html_table(csv_filepath: str) -> str:
    ...
    try:
        ...
        if missing_cols:
            return format_error_html(f"CSV is missing required columns: {', '.join(missing_cols)}")
        ...
        # Assumes standard round order can be sorted alphabetically (adjust if needed)
        df.sort_values(by=['TournamentName', 'Round'], inplace=True)

        # Iterate through DataFrame rows (each row is a match)
        for _, row in df.iterrows():
            # Format data for display, handling potential None/NaN
            p1_prob_str = f"{row['Player1_Match_Prob']:.1f}%" if pd.notna(row['Player1_Match_Prob']) else "-"
            p1_odds_str = f"{row['Player1_Match_Odds']:.2f}" if pd.notna(row['Player1_Match_Odds']) else "-"
            p2_prob_str = f"{row['Player2_Match_Prob']:.1f}%" if pd.notna(row['Player2_Match_Prob']) else "-"
            p2_odds_str = f"{row['Player2_Match_Odds']:.2f}" if pd.notna(row['Player2_Match_Odds']) else "-"

            # Escape player names and other text data to prevent HTML injection
            p1_name = pd.io.formats.html.escape(row['Player1Name'])
            p2_name = pd.io.formats.html.escape(row['Player2Name'])
            round_val = pd.io.formats.html.escape(row['Round'])
            tournament_val = pd.io.formats.html.escape(row['TournamentName'])

            # Build the HTML table row
            html_rows.append("<tr>")
            html_rows.append(f"<td>{p1_name}</td>")
            html_rows.append(f"<td style='text-align: right;'>{p1_prob_str}</td>") # Align numbers right
            html_rows.append(f"<td style='text-align: right;'>{p1_odds_str}</td>") # Align numbers right
            html_rows.append("<td style='text-align: center; font-weight: bold;'>vs</td>") # Center 'vs'
            html_rows.append(f"<td>{p2_name}</td>")
            html_rows.append(f"<td style='text-align: right;'>{p2_prob_str}</td>") # Align numbers right
            html_rows.append(f"<td style='text-align: right;'>{p2_odds_str}</td>") # Align numbers right
...
def update_index_html(template_path: str, output_path: str, table_html_content: str):
    ...
    try:
        ...
        if table_placeholder_found and timestamp_placeholder_found:
            ...
            print("ERROR: Placeholders not found. Writing original template content back.")
...
if __name__ == "__main__":
    ...
    # *** Ensure CSV_PATTERN matches the new filename base ***
    ...
    # *** Ensure TEMPLATE_FILE uses comment placeholders ***
